...
module SmlibSegment
  class Analytics
    describe Worker do
      before do
        SmlibSegment::Analytics::Transport.stub = true
      end

      describe '#init' do
        it 'accepts string keys' do
          queue = Queue.new
          worker = SmlibSegment::Analytics::Worker.new(queue,
                                                       'secret',
                                                       'batch_size' => 100)
      ...
      describe '#run' do
        before :all do
          SmlibSegment::Analytics::Defaults::Request::BACKOFF = 0.1
        end

        after :all do
          SmlibSegment::Analytics::Defaults::Request::BACKOFF = 30.0
        end

        it 'does not error if the request fails' do
          expect do
            SmlibSegment::Analytics::Transport
              .any_instance
              .stub(:send)
              .and_return(SmlibSegment::Analytics::Response.new(-1, 'Unknown error'))

            queue = Queue.new
            queue << {}
            worker = SmlibSegment::Analytics::Worker.new(queue, 'secret')
            worker.run

            expect(queue).to be_empty

            SmlibSegment::Analytics::Transport.any_instance.unstub(:send)
        ...
        it 'executes the error handler if the request is invalid' do
          SmlibSegment::Analytics::Transport
            .any_instance
            .stub(:send)
            .and_return(SmlibSegment::Analytics::Response.new(400, 'Some error'))
      ...
      describe '#is_requesting?' do
        it 'does not return true if there isn\'t a current batch' do
          queue = Queue.new
          worker = SmlibSegment::Analytics::Worker.new(queue, 'testsecret')
        ...
        it 'returns true if there is a current batch' do
          SmlibSegment::Analytics::Transport
            ...
            .stub(:send) {
              sleep(0.2)
              SmlibSegment::Analytics::Response.new(200, 'Success')
            }
